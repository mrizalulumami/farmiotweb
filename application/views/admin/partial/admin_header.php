<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <!-- google fonts -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link
        href="https://fonts.googleapis.com/css2?family=Poppins:ital,wght@0,200;0,300;0,400;0,500;1,200;1,300;1,400;1,500&display=swap"
        rel="stylesheet">

    <style>
        * {
            font-family: 'Poppins', sans-serif;
        }
    </style>

     <!-- bootstraps -->
     <link rel="stylesheet" href="<?= base_url('assets/');?>assets/css/plugin/bootstrap.min.css">
    <!-- main css -->
    <link rel="stylesheet" href="<?= base_url('assets/');?>assets/css/main.css">
    <link rel="stylesheet" href="<?= base_url('assets/');?>assets/css/plugin/circliful.css">
    <link rel="stylesheet" href="<?= base_url('assets/');?>assets/css/plugin/bootstrap.min.css">

    <!-- fontawesome -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/css/all.min.css">

    <title>Farmiot : <?= $active?></title>
</head>

<body>
        